...
describe("CA境界：2026年の連邦税バンド", () => {
  ...
  it("オンタリオ州・ケベック州・BC州・アルバータ州・マニトバ州税が実装され、他地域は未対応として残る", () => {
    expect(tax.province.implemented).toBe(true);
    expect(tax.province.implementedRegions).toEqual(["ON", "QC", "BC", "AB", "MB"]);
    expect(tax.region).toMatch(/Manitoba/);
  });
...
describe("CA境界：未実装項目が明示されている", () => {
  ...
  it("RRIF最低取崩し率への配偶者年齢の選択は実装済みで、未実装扱いにしない", () => {
    ...
    expect(all).not.toMatch(/spousal age election/i);
    expect(all).not.toMatch(/配偶者年齢.*未実装/);
  ...
  it("州税・QPPは未実装として列挙され、PAはRRSP枠計算に実装済み", () => {
    ...
    expect(all).toMatch(/オンタリオ州・ケベック州・ブリティッシュコロンビア州・アルバータ州・マニトバ州以外の州・準州所得税/);
