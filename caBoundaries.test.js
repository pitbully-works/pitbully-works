...
describe("CA境界：2026年の連邦税バンド", () => {
  ...
  it("オンタリオ州・ケベック州・BC州・アルバータ州税が実装され、他地域は未対応として残る", () => {
    expect(tax.province.implemented).toBe(true);
    expect(tax.province.implementedRegions).toEqual(["ON", "QC", "BC", "AB"]);
    expect(tax.region).toMatch(/Alberta/);
  });
...
describe("CA境界：未実装項目が明示されている", () => {
  ...
  it("州税・QPPは未実装として列挙され、PAはRRSP枠計算に実装済み", () => {
    ...
    expect(all).toMatch(/オンタリオ州・ケベック州・ブリティッシュコロンビア州・アルバータ州以外の州・準州所得税/);
