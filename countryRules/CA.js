...
export const CA_COUNTRY_RULES = {
  meta: {
    ...
    coverage: [
      ...
      { key: "tax", labelJa: "税金", labelEn: "Tax", status: "partial", effective: "2026 tax year", lastUpdated: "2026-08-22", updateJa: "連邦所得税に加え、オンタリオ州・ケベック州・ブリティッシュコロンビア州・アルバータ州・マニトバ州・サスカチュワン州の所得税、Quebec abatement、CPP/QPP・EI・QPIPを反映。その他7地域、配当税額控除・AMT等は未実装。", updateEn: "Federal income tax plus Ontario, Quebec, British Columbia, Alberta, Manitoba and Saskatchewan income tax, the Quebec abatement, CPP/QPP, EI and QPIP are modelled; the other 7 regions, dividend credits and AMT remain unimplemented." },
  ...
  tax: {
    ...
    // 2026-08-22時点：Ontario / Quebec / British Columbia / Alberta / Manitoba / Saskatchewan を自動計算。
    region: "Federal + Ontario + Quebec + British Columbia + Alberta + Manitoba + Saskatchewan",
    province: {
      implemented: true,
      implementedRegions: ["ON", "QC", "BC", "AB", "MB", "SK"],
      defaultRegion: "ON",
      ontario: {
        bands: [
    ...
    calculateProvincialCapitalGainsTax(gain, otherIncome, provinceCode = "ON") {
      ...
      if (g <= 0) return 0;
      const code = String(provinceCode || "ON").toUpperCase();
      if (!["ON", "QC", "BC", "AB", "MB", "SK"].includes(code)) return 0;
    ...
    notImplemented: [
      "オンタリオ州・ケベック州・ブリティッシュコロンビア州・アルバータ州・マニトバ州・サスカチュワン州以外の州・準州所得税（7地域。各地域で税率・バンド・控除が異なる）",
