...
describe("rule-source status atomic validation", () => {
  it("accepts valid rows and pins the URL to the bundled registry", () => {
    const out = normalizeRuleSourceStatusFeed({ schemaVersion: 1, checkedAt, sources: [
      { id: "JP-ONE", country: "jp", changed: true, hash, checkedAt, error: "", url: "https://evil.invalid" },
      { id: "US-TWO", country: "US", changed: false, hash, checkedAt, error: "" },
    ] }, registry);
    expect(out).toEqual([
      { id: "JP-ONE", country: "JP", changed: true, hash, checkedAt, url: "https://example.com/one" },
      { id: "US-TWO", country: "US", changed: false, hash, checkedAt, url: "https://example.com/two" },
    ]);
  });

  it("rejects the whole feed when an unknown source is mixed with a valid one", () => {
    expect(normalizeRuleSourceStatusFeed({ schemaVersion: 1, checkedAt, sources: [
