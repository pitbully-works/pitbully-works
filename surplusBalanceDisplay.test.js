...
describe("余剰金の積み上がり（A〜G）", () => {
  ...
  it("生活防衛資金・現在自由に使える金額は廃止：関連キーが ja・en の両方から消えている", () => {
...
// ============================================================================
// 画面データ契約：現在使える資産
//   【Ver.1.0】生活防衛資金の入力欄と「現在自由に使える金額」カードは廃止したため、
//   ここで検証するのは integrated.yearly[0].accessibleAssets を単一計算源として
//   読み出すことだけ。エンジンには何も渡さない。
// ============================================================================
describe("現在使える資産（accessibleAssets の単一計算源）", () => {
  ...
  it("accessibleAssets は 0円未満にならない（全年齢）", () => {
    const res = runIntegratedPlan(flat());
    res.yearly.forEach((r) => {
      assert.ok((r.accessibleAssets ?? 0) >= 0, `age ${r.age}: accessibleAssets が負`);
    });
  });
});
...
// 表示専用ガードレール：availableAtAge はどの入力でも負にならない。
