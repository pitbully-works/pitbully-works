...
//   ブロック2 2A〜2F：エンジンの一時支出は surplusBalance までに制限し、超過分を
//                    通常預金から引かないこと。
...
describe("余剰金の積み上がり（A〜G）", () => {
  ...
  it("C: 余剰48万・30万使用 → 余剰18万・銀行残高も30万だけ減る", () => {
    const r = runIntegratedPlan(flat({ oneTimeExpenses: [{ age: 67, amount: 300000, id: "u" }] }));
    const base0 = runIntegratedPlan(flat());
    assert.ok(near(surplusAtAge(r, 67), 180000), `余剰=${surplusAtAge(r, 67)}`);
    assert.ok(near(bankOf(base0, 67) - bankOf(r, 67), 300000), `銀行差=${bankOf(base0, 67) - bankOf(r, 67)}`);
  });

  it("D: 余剰48万・70万使用 → 実使用48万・余剰0・不足22万・通常預金に波及しない", () => {
    const r = runIntegratedPlan(flat({ oneTimeExpenses: [{ age: 67, amount: 700000, id: "u" }] }));
    const base0 = runIntegratedPlan(flat());
    const x = r.oneTimeExpenseResults[0];
    assert.ok(near(x.actuallySpent, 480000), `実使用=${x.actuallySpent}`);
    assert.ok(near(surplusAtAge(r, 67), 0), `余剰=${surplusAtAge(r, 67)}`);
    assert.ok(near(x.insufficientSurplusAmount, 220000), `不足=${x.insufficientSurplusAmount}`);
    assert.ok(near(bankOf(base0, 67) - bankOf(r, 67), 480000), `銀行は48万だけ減る=${bankOf(base0, 67) - bankOf(r, 67)}`);
  });
...
//   純粋関数 freeToSpendNow で差し引く。エンジンには渡さない。
// ============================================================================
describe("現在自由に使える金額（accessibleAssets × emergencyFund × 予定支出）", () => {
  ...
  it("自由に使える金額 = accessibleAssets − emergencyFund（0未満にしない）", () => {
    const res = runIntegratedPlan(flat());
    // 【Ver.1.0】余剰金の使用登録は廃止したため、予定支出は常に0。
    const free = freeToSpendNow({
      accessibleAssets: res.yearly[0].accessibleAssets, // = 300万
      emergencyFund: 1000000,
    });
    // 300万 − 100万 = 200万
    assert.ok(near(free, 2000000), `自由に使える=${free}`);
  });

  it("翻訳キー emergencyFund / freeToSpend が ja・en に存在する", () => {
...
describe("表示専用ガードレール（0円フロア・エンジン非依存）", () => {
  it("freeToSpendNow は生活防衛資金・予定支出が過大でも0円未満にならない", () => {
    expect(freeToSpendNow({ accessibleAssets: 1000000, emergencyFund: 9e9 })).toBe(0);
  });
...
describe("トップ統合ダッシュボードのデータ契約（単一 integrated 源）", () => {
  ...
  it("現在自由に使える金額カード = freeToSpendNow(accessible, 生活防衛)（単一源・0円フロア）", () => {
    const free = freeToSpendNow({ accessibleAssets: res.yearly[0].accessibleAssets, emergencyFund });
    // 1300万 − 100万 = 1200万（予定支出の概念は廃止）
    assert.ok(near(free, 12000000), `free=${free}`);
