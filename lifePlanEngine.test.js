...
// ===========================================================================
// 一時支出 — 指定年齢で「余剰金の範囲だけ」銀行から差し引くエンジンの汎用機能。
// 【新仕様】実際に使える額 = min(要求額, その時点の surplusBalance, 利用可能な銀行残高)。
//   要求額が余剰金残高を超えても、超過分を通常の銀行預金からは引かない。
//   結果（要求額・実使用額・不足額）を oneTimeExpenseResults に返す。
//   surplusBalance からの減算も、銀行取り崩しの一元管理を通す（generation累計ではなく実残高）。
// oneTimeExpenses が未指定/空なら従来と完全一致。
// これらのテストは「余剰金が積み上がっている状態」を作ってから使う点が旧テストと異なる。
// ===========================================================================

// 年金 月22万・生活費 月20万 → 年24万の余剰が積み上がる共通シナリオ。
const surplusCfg = (over = {}) => ({
...
test("4a②: 余剰金の範囲内の一時支出で、総資産・純資産・余剰金がその分だけ減る", () => {
  // 67歳時点の余剰は 24万×2年 = 48万。その範囲内の 30万を使う。
  const res = runIntegratedPlan(surplusCfg({ oneTimeExpenses: [{ age: 67, amount: 300000, id: "u" }] }));
  const base0 = runIntegratedPlan(surplusCfg());
  const at = (r, a) => r.yearly.find((x) => x.age === a);
  // 66歳（使用前）は基準と一致。
  assert.ok(Math.abs(at(res, 66).bankValue - at(base0, 66).bankValue) < 1, "67より前は減らない");
  // 67歳で銀行・総資産・純資産が「実使用額 30万」だけ基準より少ない。
  assert.ok(Math.abs((at(base0, 67).bankValue - at(res, 67).bankValue) - 300000) < 1, `銀行差=${at(base0, 67).bankValue - at(res, 67).bankValue}`);
  assert.ok(Math.abs((at(base0, 70).totalAssets - at(res, 70).totalAssets) - 300000) < 1, "総資産も30万減る");
  assert.ok(Math.abs((at(base0, 70).netWorth - at(res, 70).netWorth) - 300000) < 1, "純資産も30万減る");
  // 余剰金残高も 30万ぶん減る（48万 → 18万）。
  assert.ok(Math.abs(at(res, 67).surplusBalance - 180000) < 1, `67歳 余剰=${at(res, 67).surplusBalance}（期待180,000）`);
  assert.ok(Math.abs(res.cumulativeOneTimeSpent - 300000) < 1, `支出累計=${res.cumulativeOneTimeSpent}`);
  const r = res.oneTimeExpenseResults[0];
  assert.ok(Math.abs(r.actuallySpent - 300000) < 1 && Math.abs(r.insufficientSurplusAmount) < 1, "実使用30万・不足0");
  assertInvariants(res, "4a②");
});

test("4a③: 引き落としは銀行プールだけ。他の資産（投資）には波及しない", () => {
  const res = runIntegratedPlan(surplusCfg({
    pools: [
    ...
    oneTimeExpenses: [{ age: 67, amount: 300000 }],
  }));
  const base0 = runIntegratedPlan(surplusCfg({
    pools: [
      { id: "bank", group: "bank", balance: 5000000, annualReturnPct: 0, drawOrder: 1 },
  ...
  const last = res.yearly[res.yearly.length - 1];
  const lastB = base0.yearly[base0.yearly.length - 1];
  assert.ok(Math.abs((lastB.bankValue - last.bankValue) - 300000) < 1, `銀行だけ30万減る: ${lastB.bankValue - last.bankValue}`);
  assert.ok(Math.abs(last.investmentValue - 5000000) < 1, `投資は不変: ${last.investmentValue}`);
  assertInvariants(res, "4a③");
});

test("4a④【新仕様】要求額が余剰金残高を超えたら余剰金までに制限し、超過分は通常預金から引かない", () => {
  // 66歳時点の余剰は 24万。銀行残高は潤沢（1000万）だが、50万を要求しても実使用は 24万まで。
  const res = runIntegratedPlan(surplusCfg({ oneTimeExpenses: [{ age: 66, amount: 500000, id: "u" }] }));
  const base0 = runIntegratedPlan(surplusCfg());
  const at = (r, a) => r.yearly.find((x) => x.age === a);
  // 総資産・銀行は「余剰24万」ぶんだけ減る（50万ではない＝通常預金は温存）。
  assert.ok(Math.abs((at(base0, 66).bankValue - at(res, 66).bankValue) - 240000) < 1, `銀行差=${at(base0, 66).bankValue - at(res, 66).bankValue}（期待240,000）`);
  assert.ok(Math.abs(at(res, 66).surplusBalance) < 1, "66歳 余剰は0");
  const r = res.oneTimeExpenseResults[0];
  assert.ok(Math.abs(r.requestedAmount - 500000) < 1, "要求額50万");
  assert.ok(Math.abs(r.actuallySpent - 240000) < 1, `実使用=${r.actuallySpent}（期待240,000）`);
  assert.ok(Math.abs(r.insufficientSurplusAmount - 260000) < 1, `未処理=${r.insufficientSurplusAmount}（期待260,000）`);
  assert.ok(Math.abs(res.cumulativeOneTimeSpent - 240000) < 1, "累計も24万");
  assertInvariants(res, "4a④");
});

test("4a⑤: 再実行しても銀行残高の系列が完全一致（再実行で二重に引かれない）", () => {
  const cfg = surplusCfg({ oneTimeExpenses: [{ age: 67, amount: 300000 }] });
  const a = runIntegratedPlan(cfg);
  ...
  assert.equal(
    JSON.stringify(a.oneTimeExpenseResults),
    JSON.stringify(b.oneTimeExpenseResults),
    "再実行で結果が変わってはいけない"
  );
});
...
test("4a⑥: フル構成＋複数の一時支出でも全年齢で不変条件が保たれる", () => {
  ...
  assert.equal(res.yearly[0].age, 58);
  assert.equal(res.yearly[res.yearly.length - 1].age, 95);
  // 余剰金残高は常に 0 以上かつ銀行残高以下（不変条件）。
  res.yearly.forEach((r) => {
    assert.ok(r.surplusBalance >= -1e-6, `age ${r.age}: 余剰が負`);
    assert.ok(r.surplusBalance <= r.bankValue + 1, `age ${r.age}: 余剰 > 銀行`);
  });
  assertInvariants(res, "4a⑥");
});
