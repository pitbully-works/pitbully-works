...
// ============================================================================
// ---------- 国際化（i18n）基盤 ----------
// 目的：JP / US / GB / CA / AU の5か国で、表示・通貨・制度計算を国別に切り替える。
// 国固有の制度値は countryRules/、表示文言は translations/ に集約し、
// 国切替時に日本の制度値・入力データが他国へ混入しない構成とする。
// ============================================================================
...
const DEFAULT_INPUTS = {
    country: "JP", // 国別表示・通貨・制度計算・保存プロファイルの切替に使用
...
export default function NisaLifePlan({ onOpenBlog } = {}) {
  ...
  const [expandedRuleDiffs, setExpandedRuleDiffs] = useState({});
  ...
  useEffect(() => {
    if (!showRuleUpdates) return undefined;
    const previousOverflow = document.body.style.overflow;
    const onKeyDown = (event) => {
      if (event.key === "Escape") setShowRuleUpdates(false);
    };
  ...
  const importBackup = () => {
    ...
    try {
      ...
      if (isKakeiboPayload(parsed)) {
        /* いちばん先に、金額の単位を確かめる。
           家計簿は内部を最小通貨単位（円／セント）で持ち、こちらへ渡すときだけ
           主単位へ戻して amount_unit: "major" を添える決まりになっている。
           印が無い・別の値なら $12.34 が $1,234.00 になる恐れがあるので、
           **プロファイルに触れる前に** ここで止める。
           倍率を推測して直したりはしない（金額の値からは単位が分からないため）。 */
        const unit = checkKakeiboAmountUnit(parsed);
        if (!unit.ok) {
          throw new Error(
            unit.reason === "missing"
              ? t("importUnitMissingError")
              : t("importUnitUnknownError", { unit: unit.unit })
          );
        }
        ...
        const targetBase = rawTarget
          ? forceCountryMeta(applySharedIdentity(mergeSavedInputs(blank, rawTarget), inputs), target)
          : blank;
  ...
  return (
    ...
    <div className="app">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .section-block .section-title {
          padding-bottom: 11px;
          margin-bottom: 16px;
          border-bottom: 1px solid rgba(160,190,205,0.20);
        }
        .section-block .section-title .section-index {
          color: var(--section-accent);
          border-color: color-mix(in srgb, var(--section-accent) 68%, #29363C);
          ...
          {(countryRuleUpdates.length > 0 || monitoredRuleSources.length > 0) && (() => {
            ...
            return (
              <button
                ...
                style={{
                  borderColor: ruleStatusColor,
                  background: ruleStatusBackground,
                  color: ruleStatusColor,
                  fontWeight: 800,
                  whiteSpace: "nowrap",
                }}
              >
                {hasUnreviewedRuleUpdate
          ...
          {(countryRuleUpdates.length > 0 || monitoredRuleSources.length > 0) && (
            <div className="no-print" style={{ display: "flex", gap: 7, flexWrap: "wrap", alignItems: "center", fontSize: 10 }}>
              {ruleAttentionCount > 0 ? (
                ...
                <span style={{ color: "#9AA6AD", fontWeight: 800 }}>
      ...
      {(countryRuleUpdates.length > 0 || monitoredRuleSources.length > 0) && showRuleUpdates && (
        <>
          <div
            className="no-print"
            aria-hidden="true"
          ...
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label={language === "ja" ? "制度更新センター" : "Rules update center"}
            style={{
              position: "fixed",
              zIndex: 1999,
              top: "7vh",
          ...
          <div style={{ marginTop: 12 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12, fontSize: 10 }}>
                <span style={{ color: ruleAttentionCount > 0 ? "#E06B5A" : "#9AA6AD", fontWeight: 800 }}>
                  {ruleAttentionCount > 0
                    ? (language === "ja" ? `🔴 新しい制度更新 ${ruleAttentionCount}件` : `🔴 New rule updates: ${ruleAttentionCount}`)
                  ...
                  <div id={`rule-update-${update.id}`} key={update.id} style={{ border: "2px solid #344B57", borderLeft: `8px solid ${categoryAccent}`, borderRadius: 12, padding: 14, marginTop: 24, marginBottom: 6, background: "#11191E", boxShadow: "0 7px 22px rgba(0,0,0,0.26)" }}>
                    ...
                    <div style={{ marginTop: 9 }}>
                      <button
                        ...
                        onClick={() => setExpandedRuleDiffs((prev) => ({ ...prev, [update.id]: !prev[update.id] }))}
                        ...
                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                          ...
                          <tbody>{(update.changes || []).map((change, idx) => (
                            <tr key={`${update.id}-${idx}`} style={{ borderTop: "1px solid var(--line)" }}>
                              ...
                              <td style={{ textAlign: "center", fontWeight: 800, color: "#F3F8FA" }}>{formatRuleChangeValue(change.after, change.unit)}</td>
                            </tr>
                          ))}</tbody>
                        </table>
                      </div>
              ...
              <div style={{ borderTop: "1px solid var(--line)", marginTop: 14, paddingTop: 12 }}>
                ...
                {countryRuleUpdateHistory.length > 0 && (
                  <button type="button" className="history-action" onClick={clearCountryRuleUpdateHistory} style={{ marginLeft: 8, color: "#E6A092", borderColor: "#8F5147" }}>
                    {language === "ja" ? "履歴を全削除" : "Clear history"}
                  </button>
                )}
                      ...
                      [...countryRuleUpdateHistory].reverse().slice(0, 20).map((entry) => {
                        ...
                        <div key={entry.id} style={{ border: "1px solid var(--line)", borderRadius: 6, padding: "8px 10px", background: "#182027" }}>
                          ...
                          <div style={{ display: "flex", gap: 7, flexWrap: "wrap", marginTop: 7 }}>
                            ...
                            {update && (
                              <button type="button" className="history-action" style={{ fontSize: 10 }} onClick={() => {
                            ...
                            <button type="button" className="history-action" style={{ fontSize: 10, color: "#E6A092", borderColor: "#8F5147" }} onClick={() => deleteRuleUpdateHistoryEntry(entry.id)}>
                              {language === "ja" ? "この履歴を削除" : "Delete entry"}
                            </button>
                          </div>
                        </div>
                        );
